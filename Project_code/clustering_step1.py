...
X = data1.data
labels = data1.target
X_trainset, X_test1, Y_trainset, Y_test = train_test_split(X, labels, test_size=0.3, random_state=42, shuffle=True, stratify=labels)
...
clusters = [2, 5, 10, 20, 26, 28, 35, 40, 45, 50, 60, 70, 80, 100, 150, 200]
...
#k=kneed.KneeLocator(clusters, res, direction ='decreasing')
#print("knee is",k.knee)
...
for k in clusters:
    ...
    SSE[k]['Letter'] = km_1.score(X_train)
    ll[k]['Letter'] = gmm_models1.score(X_train)
    acc[k]['Letter']['Kmeans'] = cluster_acc(Y_train,km_1.predict(X_train))
    acc[k]['Letterr']['GMM'] = cluster_acc(Y_train,gmm_models1.predict(X_train))
    adjMI[k]['Letter']['Kmeans'] = ami(Y_train,km_1.predict(X_train))
    adjMI[k]['Letter']['GMM'] = ami(Y_train,gmm_models1.predict(X_train))
...
#k=kneed.KneeLocator(clusters, res, direction ='decreasing')
#print("knee for KMeans score is",k.knee)
...
#k=kneed.KneeLocator(clusters, res, direction ='decreasing')
#print("knee for GMM score is",k.knee)
plt.xlabel('Number of clusters')
...
acc.ix[:,:,'Letter'].to_csv(out+'Letter_train_acc1.csv')
adjMI.ix[:,:,'Letter'].to_csv(out+'Letter_train_adjMI1.csv')

#AIC,BOC for GMM
plt.figure(4)
n_components_range = [2,5,10,15,25,30,40,45]
...
for n_clusters in range_n_clusters:
    ...
    val_labels = clusterer.predict(X_val)
...
bench_k_means(KMeans(init='k-means++', n_clusters=40, n_init=10),
              name="k-means++", data=X_train)
#n-init = number of runs, the best run is considered in terms of inertia
bench_k_means(KMeans(init='random', n_clusters=40, n_init=10),
              name="random", data=X_train)

print(82 * '_')
print("TESTING")
print('init\t\ttime\tinertia\thomo\tcompl\tv-meas\tARI\t\tAMI\tsilhouette')
def test_k_means(estimator, name, data):
    t0 = time.time()
    estimator.fit(data)
    print('%-9s\t%.2fs\t%i\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f'
          % (name, (time.time() - t0), estimator.inertia_,
             metrics.homogeneity_score(Y_test, estimator.predict(X_test)),
             metrics.completeness_score(Y_test, estimator.predict(X_test)),
             metrics.v_measure_score(Y_test, estimator.predict(X_test)),
             metrics.adjusted_rand_score(Y_test, estimator.predict(X_test)),
             metrics.adjusted_mutual_info_score(Y_test, estimator.predict(X_test)),
             metrics.silhouette_score(X_test, estimator.predict(X_test),
                                      metric='euclidean',
                                      sample_size=sample_size)))
...
bench_gmm(GMM( n_components=30,covariance_type='full', random_state=0,init_params='kmeans'), name="GMM", data=X_train)
bench_gmm(GMM( n_components=30,covariance_type='full', random_state=0,init_params='random'), name="GMM", data=X_train)


clusters = [2, 5, 10, 20, 26, 28, 35, 40, 45]
km_model = KMeans(init='k-means++', n_clusters=40, n_init=10)
km_model.fit(X_scaled)
pred = km_model.predict(X_scaled)[...,None]
X_scaled1= np.append(X_scaled, pred,1)
...
tmp = pd.DataFrame(gs.cv_results_)
tmp.to_csv(out + 'Letters_cluster_Kmeans_A1.csv')

clusters = [2, 5, 10, 20, 26, 28, 35, 40, 45, 50, 60, 70, 80, 100]
alphas = [0.1]
architectures = [(42,)]
grid = {'gmm__n_components': clusters, 'NN__alpha': alphas, 'NN__hidden_layer_sizes': architectures}
mlp = MLPClassifier(activation='relu', max_iter=2000, early_stopping=True, random_state=5)
...
gs.fit(X_scaled,Y_trainset)
tmp = pd.DataFrame(gs.cv_results_)
tmp.to_csv(out + 'Letters_cluster_GMM_A1.csv')
